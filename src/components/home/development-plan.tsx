    ...
    <section className="py-8 md:py-12 xl:py-25 px-6 md:px-8 xl:px-32 gap-4 md:gap-6 flex flex-col xl:flex-row justify-center items-stretch">
      ...
      <Card className="flex flex-col justify-center p-4 md:p-6 xl:p-8 max-w-full xl:max-w-146 items-center w-full xl:flex-1 bg-[#F5F5F5] border border-[#E0E0E0] rounded-5 shadow-none min-h-[300px] xl:max-h-104">
        ...
        <CardContent className="p-0 flex items-center justify-center h-full">
          <p className="text-sm md:text-base leading-5 md:leading-6 font-normal text-[#4A4A4A] text-center xl:text-left">
      ...
      <Card className="w-full max-w-full xl:max-w-155 xl:flex-1">
        <div className="flex justify-center relative w-full h-[300px] md:h-[400px] xl:h-[390px] p-4 md:p-6">
          ...
          {/* Expand Button with Dialog */}
          <Dialog>
            <DialogTrigger asChild>
              <Button className="absolute top-6 md:top-8 hidden md:flex justify-center cursor-pointer items-center gap-2 right-6 md:right-8 w-full max-w-28 text-black border-[#D8D8D8] h-10 text-base font-medium bg-white border border-gray-300 rounded-full shadow hover:bg-gray-50 transition-colors">
                <Expand className="h-4 w-4" />
                Expand
              </Button>
            </DialogTrigger>

            <DialogContent className="p-0 [&>button]:cursor-pointer max-w-[50vw] max-h-[90vh] flex justify-center items-center bg-transparent border-none shadow-none">
              <img
                src="/assets/img/ic_site_development.png"
                alt="Expanded Site Development"
                className="object-contain rounded-md w-full h-full"
              />
            </DialogContent>
          </Dialog>
