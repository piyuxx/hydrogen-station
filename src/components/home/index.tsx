...
const HeroSection = () => {
  return (
    <section
      className="relative lg:min-h-screen bg-white 
      pt-[116px] md:pt-79.25  
      px-[25px] 
      pb-23.25
      lg:pb-0
    ...
    >
      <div className="container z-10 relative flex flex-col lg:flex-row items-center lg:items-stretch lg:justify-center gap-10 lg:gap-8.5">
        ...
        >
          {stats.map((item, index) => (
            <Card
              ...
              gap-2.25 w-full md:w-[226px] h-[110px] md:h-[125px] 
              rounded-[20px] 
              max-w-45.25
            ...
            >
              <div className="flex items-center gap-2">
                <h2
                  ...
                  text-[40px] md:text-[48px] lg:text-[54px]"
                >
                  {item.number}
                </h2>
                {item?.label && (
                  <p className="text-[12px] md:text-[13px]">{item.label}</p>
                )}
              </div>
              {item?.subLabel && (
                <span className="text-[12px] font-medium">{item.subLabel}</span>
              )}
            </Card>
          ))}
        </div>
